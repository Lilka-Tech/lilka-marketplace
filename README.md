...
Authors add exactly one `catalog/entries/<sha256(type + ':' + id + '@' + version)>.json` containing a V2 reference. Package bytes remain in the publisher repository. Existing versions are immutable; publish a new version instead. The signed publisher compiles all registry entries into the unchanged V2 index/typed catalogs and signs them atomically. PR validation intentionally allows generated indexes to lag new references. Different authors no longer edit a shared array. Do not edit generated catalog indexes in submissions.
